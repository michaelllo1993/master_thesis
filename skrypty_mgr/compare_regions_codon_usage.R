...
for (i in seq(1,dim(codons_count_lsaar_fraction)[1])){
  svg(filename=paste(wd,"/",rownames(codons_count_lsaar_fraction)[i],"_codon_usage_region.svg",sep = ""), 
      width=40, 
      height=30, 
      pointsize=50)
  barplot(rbind(codons_count_protein_fraction[i,]*100,codons_count_sigp_fraction[i,]*100,codons_count_lsaar_fraction[i,]*100),beside = T,col = c("dodgerblue3","firebrick2","darkgreen"),xlab = "Codon",ylab = "Codon usage [%]",main = paste("L codon usage for", rownames(codons_count_lsaar_fraction)[i]),ylim = c(0,100))
  legend("topleft",c("The rest of the protein","Signal peptide","L-SAAR"),col = c("dodgerblue3","firebrick2","darkgreen"),pch = 15,bty = "n")
  dev.off()
}
