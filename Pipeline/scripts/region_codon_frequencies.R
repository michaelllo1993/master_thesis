...
for (k in 1:length(myFiles_overall)) {
  overall_names[k] = paste(strsplit(myFiles_overall[k],"_")[[1]][1],strsplit(myFiles_overall[k],"_")[[1]][2],sep = "_")
  overall[[k]] = read.csv(myFiles_overall[k], stringsAsFactors = F,header = T)
...
for (k in 1:length(myFiles_overall_noSP)) {
  overall_noSP_names[k] = paste(strsplit(myFiles_overall_noSP[k],"_")[[1]][1],strsplit(myFiles_overall_noSP[k],"_")[[1]][2],sep = "_")
  overall_noSP[[k]] = read.csv(myFiles_overall_noSP[k], stringsAsFactors = F,header = T)
...
for (k in 1:length(myFiles_withinSP)) {
  withinSP_names[k] = paste(strsplit(myFiles_withinSP[k],"_")[[1]][1],strsplit(myFiles_withinSP[k],"_")[[1]][2],sep = "_")
  withinSP[[k]] = read.csv(myFiles_withinSP[k], stringsAsFactors = F,header = T)
...
for (k in 1:length(myFiles_withinSAAR)) {
  withinSAAR_names[k] = paste(strsplit(myFiles_withinSAAR[k],"_")[[1]][1],strsplit(myFiles_withinSAAR[k],"_")[[1]][2],sep = "_")
  withinSAAR[[k]] = read.csv(myFiles_withinSAAR[k], stringsAsFactors = F,header = T)
...
codons_protein_fraction = matrix(NaN,nrow = length(overall_noSP) ,ncol = length(unit_codons))
...
codons_SAAR_fraction = matrix(NaN,nrow = length(withinSAAR) ,ncol = length(unit_codons))
...
for (i in seq(1, dim(codons_SAAR_fraction)[1])) {
  svg(
    filename = paste(wd,"/codon_frequency/",OoI,"_orthologues/","/plots_",OoI,"_orthologues/",
