...
#get the last argument and delete it
other_OoIs =  args[length(args)]; args = args[-length(args)]
...
for (file_name in codon_changes_files){
  ...
  other_OoI = names(which(sapply(other_OoIs, function(x) grepl(x,file_name))))
  ...
  image_name = paste("/visualization_",OoI,"/",gsub(".csv",".svg",file_name),sep = "")
  #read the data in
  ...
  #create image and seve it 
  svg(filename=image_name,
      width=40, 
      height=30, 
      pointsize=50)
