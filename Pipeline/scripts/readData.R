...
for (org in seq(1,length(cDNA),by = 1)) {
  cDNA[[org]]$START = lapply(cDNA[[org]]$START, function(x) unlist(strsplit(x,";")[[1]]));#split by semicolon
  cDNA[[org]]$STOP = lapply(cDNA[[org]]$STOP, function(x) unlist(strsplit(x,";")[[1]]));#split by semicolon
}
