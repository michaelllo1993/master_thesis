...
my $usage = <<EOF;
USAGE: (name).pl (organism)_signalp_positives.out ensembl_parsed_(organism) repeatUnit repeatThreshold
EOF

my $sp4_positives = "$ARGV[0]" || die $usage;
my $ensembl_parsed = "$ARGV[1]" || die $usage;
my $tag = "$ARGV[2]" || die $usage;
my $tag_threshold = "$ARGV[3]" || die $usage;
