library(Biostrings)
library(seqinr)
setwd("/home/mstolarczyk/Uczelnia/MGR/praca_magisterska/codon_analysis")
...
a = matrix(
  data = NaN,
  nrow = length(cDNA$cDNA_homo_sapiens$SEQUENCE),
...
for (org in seq(1, 9, 1)) {
  for (i in seq(1, length(cDNA[[org]]$SEQUENCE))) {
    ...
    for (codon in seq(1, length(leucine_codons), by = 1)) {
      seq = c2s(s2c(cDNA[[org]]$SEQUENCE[i])[seq(sp_end * 3, length(s2c(cDNA[[org]]$SEQUENCE[i])), by =
                                                   1)])
      codons_count_protein[[org]][i, codon] = length(which(
        strsplit(seq, "(?<=.{3})", perl = TRUE)[[1]] == leucine_codons[codon]
      ))
    }
  }
...
a = matrix(
  data = NaN,
  nrow = length(sigp$sigp_ALL_homo_sapiens$SEQ),
  ncol = length(leucine_codons)
)
...
for (org in seq(1, 9, 1)) {
  for (i in seq(1, length(sigp[[org]]$SEQ))) {
    if (!length(sigpL[[org]]$cDNA_ID[which(sigpL[[org]]$cDNA_ID == sigp[[org]]$cDNA_ID[i])]) == 0) {
      ...
      for (codon in seq(1, length(leucine_codons), by = 1)) {
        seq = c2s(s2c(cDNA[[org]]$SEQ[which(cDNA[[org]]$cDNA_ID == sigp[[org]]$cDNA_ID[i])])[c(seq(1, (lsaar_start *
                                                                                                         3 + 1), 1), seq((lsaar_start * 3 + 1) + (lsaar_length * 3), length(s2c(sigp[[org]]$SEQ[i])) *
                                                                                                                           3))])
        codons_count_sigp[[org]][i, codon] = length(which(
          strsplit(seq, "(?<=.{3})", perl = TRUE)[[1]] == leucine_codons[codon]
        ))
      }
    }
      ...
      for (codon in seq(1, length(leucine_codons), by = 1)) {
        seq = c2s(s2c(cDNA[[org]]$SEQ[which(cDNA[[org]]$cDNA_ID == sigp[[org]]$cDNA_ID[i])])[seq(1, sigp[[org]]$SP_END[i] *
                                                                                                   3)])
        codons_count_sigp[[org]][i, codon] = length(which(
          strsplit(seq, "(?<=.{3})", perl = TRUE)[[1]] == leucine_codons[codon]
        ))
...
a = matrix(
  data = NaN,
  nrow = length(sigpL$sigpL_ALL_homo_sapiens$SEQ),
  ncol = length(leucine_codons)
)
  ...
  for (i in seq(1, length(sigpL[[org]]$SEQ))) {
    for (codon in seq(1, length(leucine_codons), by = 1)) {
      if (!is.na(sigpL[[org]]$LSAAR_LENGTH[i])) {
        seq = c2s(s2c(cDNA[[org]]$SEQ[which(cDNA[[org]]$cDNA_ID == sigpL[[org]]$cDNA_ID[i])])[seq((sigpL[[org]]$LSAAR_START[i] *
                                                                                                     3 + 1),
                                                                                                  (sigpL[[org]]$LSAAR_START[i] * 3 + 1) + (sigpL[[org]]$LSAAR_LENGTH[i] *
                                                                                                                                             3)
        )])
        codons_count_lsaar[[org]][i, codon] = length(which(
          strsplit(seq, "(?<=.{3})", perl = TRUE)[[1]] == leucine_codons[codon]
        ))
      }
    }
...
for (i in seq(1, length(codons_count_lsaar), 1)) {
  codons_count_lsaar[[i]] = codons_count_lsaar[[i]][seq(1, length(which(!is.na(
    codons_count_lsaar[[i]][, 1]
  ))), 1),]
  codons_count_protein[[i]] = codons_count_protein[[i]][seq(1, length(which(!is.na(
    codons_count_protein[[i]][, 1]
  ))), 1),]
  codons_count_sigp[[i]] = codons_count_sigp[[i]][seq(1, length(which(!is.na(
    codons_count_sigp[[i]][, 1]
  ))), 1),]
...
# data into matrices ------------------------------------------------------
codons_count_lsaar_fraction = matrix(NaN,
                                     nrow = length(codons_count_lsaar) ,
                                     ncol = length(leucine_codons))
for (org in seq(1, length(codons_count_lsaar), 1)) {
  for (cod in seq(1, length(leucine_codons), 1)) {
    codons_count_lsaar_fraction[org, cod] = sum(codons_count_lsaar[[org]][, cod], na.rm = T) /
      sum(codons_count_lsaar[[org]], na.rm = T)
  }
}
codons_count_sigp_fraction = matrix(NaN,
                                    nrow = length(codons_count_sigp) ,
                                    ncol = length(leucine_codons))
for (org in seq(1, length(codons_count_sigp), 1)) {
  for (cod in seq(1, length(leucine_codons), 1)) {
    codons_count_sigp_fraction[org, cod] = sum(codons_count_sigp[[org]][, cod], na.rm = T) /
      sum(codons_count_sigp[[org]], na.rm = T)
  }
}
codons_count_protein_fraction = matrix(NaN,
                                       nrow = length(codons_count_protein) ,
                                       ncol = length(leucine_codons))
...
for (i in seq(1, dim(codons_count_lsaar_fraction)[1])) {
  svg(
    filename = paste(
      wd,
      "/",
      rownames(codons_count_lsaar_fraction)[i],
  ...
  barplot(
    rbind(
      codons_count_protein_fraction[i,] * 100,
      codons_count_sigp_fraction[i,] * 100,
      codons_count_lsaar_fraction[i,] * 100
    ),
    beside = T,
    col = c("dodgerblue3", "firebrick2", "darkgreen"),
    xlab = "Codon",
    ylab = "Codon usage [%]",
    main = paste("L codon usage for", rownames(codons_count_lsaar_fraction)[i]),
    ylim = c(0, 100)
  )
  legend(
    "topleft",
...
levenes_test_result = leveneTest(
  c(
    codons_count_lsaar_fraction[1:8, 6],
