...
USAGE: Needle_L_ALL_wrapper.pl <human_and_orthologues_protein_ids.tsv> <organisms> <sigpL_ALL.out>
...
close (OUT0);
print "\nEnsembl_ids_mapper.csv created in /bi/home/mstolarczyk/SAARs/Data/needle_data\n";

# Improting Ensemble ids mapper to arrays
open (IN0, "+<", "/bi/home/mstolarczyk/SAARs/Data/needle_data/Ensembl_ids_mapper.csv");
my $ii = 0;
my @homo_sapiens;
my @pan_troglodytes;
my @mus_musculus; 
...
for (my $j = 1; $j <= 6; $j++) {
	my $parsed_FILE = "ensembl_parsed_$ARGV[$j]" || die $usage;
	...
	$fasta{$ARGV[$j]}{$id}=$seq;							# the last value will be added (because the last one is not printed in the loop anymore)
...
for $ens_id (keys %sigp_ALL_L) {		
		...
		system ("cat temp_L_ALL_Hsap_Ortho.needle >> ALL_Hsap_Ortho_all.needle");
		if ($exit_stat != 0)
		{
